...
export function createMeetingRequest(title, content, date, userList, startTime, comment, partName, checkList) {
    return (dispatch) => {
        ...
        let P_url = 'http://192.168.43.115:8080/meeting/add';
...
export function meetingListLoadRequest(id) {
  return (dispatch) => {
      ...
      let P_url = 'http://192.168.43.115:8080/user/end/' + id;
      let url = 'api/user/end/' + id;
      return axios.get(P_url)
      .then((response) => {
        let end = response.data;
        console.log(response);
        let P_url = 'http://192.168.43.115:8080/user/wait/' + id;
...
export function meetingLoadRequest(data, status) {
    return (dispatch) => {
        // Inform Login API is starting
        dispatch(meetingLoad());

        dispatch(meetingLoadSuccess(data));
        if(status == 'END'){
        ...
        /*
        let P_url = 'http://192.168.43.164:8080/meeting/info/' + id.toString();
...
export function meetingSaveRequest(id, data, partName, checkList) {
    return (dispatch) => {
        ...
        let url = 'http://192.168.43.164:8088/textinsert/' + id +'/' + data + '/' + partName;
        return axios.get(url)
        .then((response) => {
          console.log(response);
          let url = 'http://192.168.43.115:8080/meeting/end/' + id;
