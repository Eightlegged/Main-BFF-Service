          ...
          {this.props.meetings.map((data, i) => {
            return (
              <li className="list-group-item" key={i}>
                <div><h4>{data.title}</h4></div><div>{data.date}</div><div className="text-right"> <button className="btn-info btn btn-xs" onClick={() => this.callMeeting(data.id)}>상세보기</button></div>
              </li>
            );
          })}
          </ul>
